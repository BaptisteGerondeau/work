...
#
# this script builds kbuild using this repo's dir structure
#
...
TARGET="$HOME/work/pkgs/$MYARCH/kbuild"
...
for dir in $DIRS; do
    ...
    if [ ! -f $TARGET/kbuild-$DESCRIBE-$MYARCH.txz ]; then

        # generating a new .txz file

        echo "INFO: kbuild $DESCRIBE-$MYARCH being generated."

        $COMPILE -C tools clean
        CFLAGS="-fPIC" $COMPILE -C tools/testing/build all
        RET=$?

        # TODO: check for compilation errors

        if [ $RET -eq 0 ]; then
            tar cfJ $TARGET/kbuild-$DESCRIBE-$MYARCH.txz ./tools
            ls $TARGET/kbuild-$DESCRIBE-$MYARCH.txz
            [ ! -f $TARGET/kbuild-$DESCRIBE-$MYARCH.txz ] && echo "ERROR: kbuild $DESCRIBE-$MYARCH not created."
        else
            echo "ERROR: kbuild $DESCRIBE-$MYARCH could not be compiled."
        fi
        ...
        echo "INFO: kbuild-$DESCRIBE-$MYARCH already exists"
    fi
