...
DIR_KERNEL_SRC="$DIR_HOME/../rpmbuild/BUILD/kernel-alt-4.14.0-115.6.1.el7a/linux-4.14.0-115.6.1.el7a.aarch64/"
...
#### E2FSPROGS BUILD ENV
DIR_E2PROGS="$DIR_HOME/e2fspross"
...
DIR_LUSTRE_SRC="$DIR_LUSTRE/lustre-release"
LUSTRE_BRANCH="v2_12_52"
...
sudo yum -y install xmlto asciidoc elfutils-libelf-devel zlib-devel binutils-devel newt-devel python-devel \
			...
			kernel-devel vim wget libaio-devel
# Kernel sources !
sudo yum --enablerepo=base-debuginfo install -y kernel-debuginfo

# Python 3 is in epel...
sudo yum -y install epel-release
sudo yum -y install --enablerepo=epel python34 python34-devel python34-setuptools python3-cffi libyaml-devel libyaml libtool

sudo yum install -y http://build.openhpc.community/OpenHPC:/1.3/CentOS_7/aarch64/ohpc-release-1.3-1.el7.aarch64.rpm || true 
sudo yum install -y ohpc-base ohpc-base-compute ohpc-lmod gnu-compilers-ohpc

# We need to reload bash for lmod to be picked up
exec bash
ml load gnu/5.4.0
...
#	&& $DIR_SPL_SRC/configure --with-config=user --with-linux=$DIR_KERNEL \
...
sudo yum -y install spl kmod-spl-devel
...
# cd $DIR_E2PROGS_SRC
# sh "$DIR_E2PROGS_SRC/autogen.sh" \
#	&& $DIR_E2PROGS_SRC/configure \
...
# Get Lustre source
git clone -b $LUSTRE_BRANCH git://git.whamcloud.com/fs/lustre-release.git $DIR_LUSTRE/lustre-release/ || true
...
sh "$DIR_LUSTRE_SRC/autogen.sh" \
	&& $DIR_LUSTRE_SRC/configure --enable-server --enable-modules \
		...
		--with-spl="$DIR_SPL_SRC" --disable-ldiskfs \
       	&& l_make $DIR_LUSTRE_SRC rpms -j $NPROC
